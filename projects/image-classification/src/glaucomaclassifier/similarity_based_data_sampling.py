...
def sample_dissimilar_images(image_embeddings: torch.Tensor, num_samples: int, index_offset: int):
...
def main():
    ...
    data_loader = DataLoader(majority_class_dataset, batch_size=BATCH_SIZE, shuffle=False)

    selected_ids = []
    with torch.no_grad():
        image_embeddings = None
        for batch_number, (inputs, labels) in tqdm(iterable=enumerate(data_loader), total=TOTAL_BATCHES + 1):
