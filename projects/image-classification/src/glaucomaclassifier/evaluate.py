...
def get_predictions(val_loader, model, device):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(iterable=val_loader, total=len(val_loader)):
            ...
            glaucoma_probs.extend(probs.cpu().numpy()[:, 1])  # Probability for class '1'
...
def sensitivity_at_certain_specificity(target_specificity, fpr, tpr):
    closest_specificity_index = np.argmin(np.abs(tpr + (1 - fpr) - (1 + target_specificity)))
    sensitivity_at_specificity = tpr[closest_specificity_index]
    print(f'Sensitivity at {target_specificity*100:.0f}% Specificity: {sensitivity_at_specificity:.2f}')
...
def main():
    ...
    # Get predictions
    original_labels, predicted_labels, glaucoma_probs = get_predictions(val_data_loader, model, device)

    # Plot and compute metrics
    plot_confusion_matrix(original_labels, predicted_labels, list(CLASS_NAME_ID_MAP.keys()))
    ...
    fpr, tpr, thresholds = roc_curve(original_labels, glaucoma_probs)
    sensitivity_at_certain_specificity(0.95, fpr, tpr)  # Change 0.95 to your target specificity
