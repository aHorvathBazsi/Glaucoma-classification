...
def get_data_loaders(train_val_ratio=0.8, max_rotation_angle=20, batch_size=32, use_weighted_sampler=False):
    ...
    train_label_dataframe, val_label_dataframe = split_label_dataframe(label_dataframe=label_dataframe, fraction=train_val_ratio)

    train_image_transform = get_image_transform(is_train=True, input_size=INPUT_SIZE, max_rotation_angle=max_rotation_angle)
    val_image_transform = get_image_transform(is_train=False, input_size=INPUT_SIZE, max_rotation_angle=max_rotation_angle)
    ...
    if use_weighted_sampler:
        train_dataset_size = train_dataset.label_dataframe['encoded_label'].value_counts().min().item() * 2
    ...
    return train_data_loader, val_data_loader, train_dataset_size, val_dataset_size, class_weigths